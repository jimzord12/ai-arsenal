...
Both interval values must be strict, possible ISO 8601 instants with `Z` or an explicit offset. The collector compares inclusive bounds against Git committer timestamps. Remote and default-branch names use a conservative alphanumeric, dot, underscore, hyphen, and slash grammar that excludes double dots and `.lock` components; URL-shaped, control-character, and otherwise rejected input is not echoed.

## Git evidence

Before reading evidence, the collector fetches and prunes the configured remote's branch heads non-interactively without tags, submodule recursion, `FETCH_HEAD` writes, automatic maintenance, branch checkouts, or working-tree changes. It rejects configured remotes whose case-folded slash-delimited name overlaps another remote's tracking-ref namespace before fetch.
...
Commit evidence contains the full 40- or 64-hex object ID, parent object IDs, subject, and strict ISO committer timestamp. Runtime validation applies the same object-ID and instant rules to branch heads, merge bases, and interval bounds. Arrays are deterministic: branches use name order and commits use reverse topological order. A verified empty interval and repository with no unmerged branches returns empty arrays.

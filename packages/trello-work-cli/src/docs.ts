...
export const DOCS_TOPICS: DocsTopic[] = [
  {
    ...
    content:
      "jz-trello-flow skills install asks Git for the actual Git top level and replaces exactly trello-work-orchestrator, trello-work-design, trello-work-deliver, and trello-work-recover beneath that repository's .agents/skills directory. It rejects a symbolic link or junction in the managed path and preserves unrelated skill directories. Every invocation stages the complete bundled payload, marks it as managed and replaceable, and validates all four skills by executing official skills-ref@0.1.0 source from a clean agentskills/agentskills checkout at commit 38a2ff82958afee88dadf4831509e6f7e9d8ef4e. Set JZ_TRELLO_FLOW_SKILLS_REF_CHECKOUT to that checkout's actual Git top level and JZ_TRELLO_FLOW_SKILLS_REF_PYTHON to a Python environment containing its dependencies; an unqualified PATH executable is never trusted. Use --dry-run to validate and report installed or replaced actions without changing the repository. If restoration fails, the error ends with Recovery data preserved at: <exact path>; retain that path for manual recovery. This offline command neither loads Trello credentials nor accesses Trello.",
  },
  {
    id: 'concepts',

...
Native Trello card members and Work Unit metadata `owner` have distinct responsibilities. Members are plural Trello accounts assigned to the card for shared responsibility and attention; `owner` is the single stable agent or worker identity holding the execution claim. Trello watching/subscription is separate from membership. `get`, `list`, and `inbox list` expose deterministic member objects containing `id`, `username`, and `fullName`. Without `--member`, `list` retains its normalized-Work-Unit-only output. `list --member <selector>` searches all visible board cards, excludes archived cards, and returns matching ordinary cards and Work Units in the established Inbox discriminated shapes. It matches only an exact member ID or case-insensitive exact username, never display names or substrings. The existing `list --owner <stable-owner>` remains an exact metadata filter; metadata filters exclude ordinary cards, and all supplied filters compose conjunctively. Reads never add, remove, or synchronize members.
...
The command is self-contained after package installation. It stages and structurally validates the complete transformed payload before replacing a target and uses only files bundled with the executing CLI version. Runtime installation needs no Python, validator checkout, network, or Trello credentials. Official `skills-ref@0.1.0` validation against the exact transformed payload is a development and release gate pinned to upstream commit `38a2ff82958afee88dadf4831509e6f7e9d8ef4e`; it is not an end-user runtime dependency. Use `--dry-run` to report each planned `installed` or `replaced` action without changing the repository.

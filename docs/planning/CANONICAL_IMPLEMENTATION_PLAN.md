...
Commit and push the exact verified archived-list canonical-resolution snapshot and confirm required GitHub Actions pass. Then install that exact captured local package globally and execute the separately authorized onboarding workbook only on TestingBoard. Release, publication, production-board access, and source deletion remain separately gated and unapproved.

...
> **Status:** Workflow v2 is delivered; reviewed Trello Flow CLI `0.4.1` reliability candidate passes package, repository, packed-consumer, and workflow gates and awaits exact-SHA CI before global replacement
> **Living-plan schema:** 1.0
> **Last reconciled:** 2026-07-30
> **Current phase:** Commit and push Trello Flow CLI `0.4.1`, require exact-SHA Quality and Portability CI, then install and verify that exact artifact globally
...
- `[VERIFIED]` The private `@jz/ai-arsenal-trello-work-cli` package is globally installed as `0.4.0`, exposing only `jz-trello-flow`. Release SHA `9818bf6a359d53d95705f0b9685416cf06837d5b` passed Quality `30467891276` and Portability `30467891911`. Its exact tarball SHA-256 is `86ad985318fd3fef537bdf544dc4f53fdc4046e07e765d92f81d041c0d8d3635`; native help/docs and disposable first/repeat transformed-byte proof pass with unrelated sentinel bytes preserved. Registry publication remains unapproved.
- `[VERIFIED]` Changesets generated the private `0.4.1` reliability candidate. It preflights every exact final Trello description against the documented 16,384-character limit with structured no-write size evidence, writes bounded versioned SHA-256 operation records while retaining exact legacy replay, reports transition dry-runs through the executable, accepts and removes only explicitly resolved Open Questions on Ready transition, and ships aligned recovery guidance in all four managed skills. The versioned candidate passes 274 package tests, strict publint, repository and workflow gates, credential-free live-harness boundaries, clean-consumer installation, generated-shim help/docs, and direct packed-source compact-record/preflight smoke. Exact-SHA CI and global `0.4.1` replacement remain pending; Greek Essence and live Trello state were not accessed.
...
- `[VERIFIED]` `jz-trello-flow skills install` is an offline, credential-free command that discovers the containing Git repository from nested paths, ignores inherited `GIT_*` repository selectors, and installs the four package-bundled managed Trello skills under `.agents/skills/`. Every run prepares and validates the full staged payload through the provenance-verified pinned `skills-ref` source before mutation, replaces only the four managed targets with rollback for later swap failure, preserves unrelated skills, and supports no-write `--dry-run` action reporting. Version `0.4.0` is installed globally and its repository-owned byte proof verifies transformed payload bytes, exact inventory, redirected-path containment, sentinel preservation, and repeat identity. Version `0.4.1` preserves byte-identical canonical/package skill sources while adding size-aware recovery guidance; registry publication remains unapproved.
...
The reviewed private Trello Flow CLI `0.4.1` candidate is versioned and passes all local package, repository, packed-consumer, and workflow gates. Commit and push the exact candidate, require successful exact-SHA Quality and Portability CI, then pack that CI-green commit, replace the global pnpm `0.4.0` installation with its exact `0.4.1` tarball, and independently verify version, shim, help/docs, compact-record/preflight behavior, and managed-skill bytes. Record post-install evidence, close the compact work item, push final current truth, and require final exact-SHA CI. Do not access or mutate Greek Essence WU-20 during this delivery.

...
- `[VERIFIED]` The private `@jz/ai-arsenal-trello-work-cli` source package is versioned `0.3.0` and exposes only the `jz-trello-flow` executable. The Changesets-generated minor release records attachment metadata and explicit authenticated downloads. Real packed-manifest coverage proves the attachment module/docs are present, tests/configuration/credentials and obsolete package-owned `work` alias are absent, and package/root/workflow gates plus independent review pass. Registry publication remains unapproved; global replacement waits for exact-SHA CI.
...
Commit and push the exact verified Trello CLI `0.3.0` release snapshot and require Quality and Portability for its full SHA. Then pack from that clean exact commit, replace the globally installed `0.2.0` with the authorized exact tarball, verify native Windows help/version, and rerun the read-only attachment workbook only on TestingBoard `6a16bbf1fea5389eb39636b7`. Registry publication, production-board access, Trello mutation, Git Bash remediation, and source deletion remain unapproved.

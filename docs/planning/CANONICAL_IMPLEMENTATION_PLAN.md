...
> **Status:** Revised Trello Work Unit intake/design lifecycle verified offline; capture and authorized TestingBoard validation remain
...
- `[VERIFIED]` The Trello CLI supports ordinary Inbox cards and canonical Draft Work Units, `work inbox list`, explicit `work draft create`, the deprecated `work create` alias, same-card `work design start` promotion, canonical partial `in_design`, and strict In Design-to-Ready completeness gating. Malformed cards claiming Work Unit identity fail closed, and design operation IDs are checked board-wide before mutation.
...
- `[VERIFIED]` Historical live validation was allowlisted to `TestingBoard` (`6a16bbf1fea5389eb39636b7`) and left all generated cards in Done with disposable lists closed. The revised seven-list onboarding harness independently initializes its preconditions, covers ordinary-card and Draft intake, uses full-card-scoped operation IDs, and is not yet executed live; global installation and revised TestingBoard validation remain the approved post-capture operation.
...
- `[VERIFIED]` The private `@jz/ai-arsenal-trello-work-cli` package is versioned `0.1.0` with a Changesets-generated changelog for archived-list canonical resolution. Its exact package identity, packed boundary, package/root/workflow gates, and independent review pass; publication remains unapproved.
...
- `work draft create` creates structured Inbox drafts, `work create` is a deprecated compatibility alias, and `work design start` converts the selected card in place so card identity and Trello history are preserved.
...
Commit and push the exact verified archived-list resolution snapshot and confirm required CI. Then globally install that captured local package and run the separately authorized onboarding workbook only on TestingBoard `6a16bbf1fea5389eb39636b7`; record credential-free evidence and stop on any board, mutation, cleanup, or command-resolution ambiguity.
...
| Revised seven-list Trello lifecycle has not yet been exercised live                                                | Git/CI capture pending            | Commit/push the verified archived-list fix, confirm CI, then install the exact captured package and run only on TestingBoard |
...
The dedicated TestingBoard identifier, process-environment credential source, and recovery-aware procedure are verified from prior work. Archived-list canonical resolution is verified offline and independently reviewed; revised seven-list and dual-intake live validation remains pending after Git/CI capture and exact-package installation. Production-board use and the production atomic ID-allocation backend remain open and separately gated; dedicated-board validation must not be represented as production-board authorization or as proving a Trello-backed concurrency mechanism.
...
Commit and push the exact verified Trello Work CLI `0.1.0` snapshot and confirm required GitHub Actions pass. Then install that exact captured local package globally and verify `work --help` before executing the separately authorized onboarding workbook only on TestingBoard. Release, publication, production-board access, and source deletion remain separately gated and unapproved.

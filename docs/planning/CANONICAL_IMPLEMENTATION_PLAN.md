...
> **Status:** Workflow v2, Trello Flow CLI `0.5.0`, and weekly-report CLI `0.1.0` are delivered and globally installed; no work item is active
...
Trello Flow CLI `0.5.0` native member output/filtering and distinct metadata-owner filtering are delivered from exact CI-green artifact commit `e9c005a2391b21c3add345203276bf720e1f48c6` and globally installed from the byte-proven artifact with retained `0.4.2` rollback. No work item is active; define the next explicit bounded request through Workflow v2.

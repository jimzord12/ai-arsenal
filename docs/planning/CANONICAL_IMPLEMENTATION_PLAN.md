...
> **Status:** Trello Flow CLI `0.5.1` all-visible/open-card member filtering is verified and in delivery; globally installed `0.5.0` remains the rollback state until exact-SHA delivery completes
...
- `[VERIFIED]` The private `@jz/ai-arsenal-trello-work-cli` package is globally installed as `0.5.0`, exposing only `jz-trello-flow`. Artifact SHA `e9c005a2391b21c3add345203276bf720e1f48c6` passed Quality `30614651328` and Portability `30614651510` on Ubuntu and Windows. Its exact 32-file tarball SHA-256 is `42e57a200b47d4bc020da186eacb7023d12148d1472cc12f422b9c94e4d88be0`; all package files byte-match the global installation, native help/docs pass, and installed-source credential-free smoke proves member-only, owner-only, combined-match, combined-mismatch, and display-name-rejection behavior. Exact `0.4.2` rollback remains available; registry publication remains unapproved.
...
- Official pinned `skills-ref` validation is a development/release gate against the exact transformed payload, not an end-user runtime dependency. Version `0.5.0` passes focused, package, official-validator, packed-consumer, root, workflow, exact-SHA CI, global-install, installed-byte, and repeat/sentinel gates without Trello or registry access.
- Version `0.5.0` adds strictly normalized native card-member output and exact member filtering while preserving owner as a separate execution-claim filter. It passes 281 package tests, strict publint, official pinned skill validation, root/workflow gates, credential-free live safety checks, a 32-file clean consumer, privacy/diff checks, exact-SHA CI, byte-proven global installation, and independent installed behavior smoke.
- Versioned source `0.5.1` corrects member filtering to include every matching visible/open ordinary card and Work Unit, with archived cards excluded at the transport boundary and metadata filters remaining Work Unit-specific. Final verification passes 281 package tests, root/workflow gates, strict publint, official pinned skill validation, credential-free live safety, a 32-file clean consumer, privacy/diff checks, and read-only TestingBoard evidence. Exact-SHA CI, exact-commit packing, and global replacement remain the active delivery boundary.

### Delivery boundary

The protocol, canonical skill sources, package payload, self-contained offline installer command, help, offline documentation, private `0.5.0` release, and global installation are complete and verified. Generated client adapters, automatic client bootstrap beyond repository-local `.agents/skills/` installation, registry publication, and production-board migration remain separate bounded work items.
...
Deliver the verified Trello Flow CLI `0.5.1` snapshot through its exact artifact-bearing commit, required exact-SHA Quality and Windows/Linux Portability CI, exact-commit tarball, retained `0.5.0` rollback, approved global replacement, installed-byte proof, and read-only TestingBoard smoke. Keep the active Workflow v2 item open until that delivery evidence is complete.

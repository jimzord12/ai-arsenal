...
> **Status:** Workflow v2 is delivered; Trello Flow CLI `0.4.0` passed release-SHA CI, exact-tarball installation, native shim smoke, and reproducible first/repeat skills-install byte proof
> **Living-plan schema:** 1.0
> **Last reconciled:** 2026-07-29
> **Current phase:** Push post-install current truth and require final exact-SHA Quality and Portability CI
...
- `[VERIFIED]` `jz-trello-flow skills install` is an offline, credential-free command that discovers the containing Git repository from nested paths, ignores inherited `GIT_*` repository selectors, and installs the four package-bundled managed Trello skills under `.agents/skills/`. Every run prepares and validates the full staged payload through the provenance-verified pinned `skills-ref` source before mutation, replaces only the four managed targets with rollback for later swap failure, preserves unrelated skills, and supports no-write `--dry-run` action reporting. Changesets generated the reviewed private `0.4.0` candidate. The actual candidate tarball passes disposable consumer installation and invocation, and the repository-owned byte proof verifies transformed payload bytes, exact inventory, redirected-path containment, sentinel preservation, and repeat identity. Exact-SHA CI and global `0.4.0` installation remain pending; no publication, Trello call, or non-disposable skill installation was performed.

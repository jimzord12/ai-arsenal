...
> **Status:** Workflow v2 review-barrier integration, fail-closed CLI delivery evidence, isolated-worktree-per-item development, legacy Features CLI junction retirement, repaired weekly-report CLI `0.1.1`, and Trello Flow CLI `0.8.0` canonical template output are delivered
> **Living-plan schema:** 1.0
> **Last reconciled:** 2026-08-05
> **Current phase:** Trello Flow CLI authoring and discovery UX continues with issue #24
...
- `[VERIFIED]` Versioned source `0.7.0` adds concise offline `--help` for every public `jz-trello-flow` command and subcommand before argument validation, configuration, credential access, board resolution, or mutation. Each output renders its catalog syntax, complete options, and one explicit pipe-free example; mutating examples include `--dry-run` and configured examples include `--board <id-or-exact-name>`. It retains credential-free top-level `jz-trello-flow -v` and `jz-trello-flow --version` output before ordinary parsing or configuration loading, and strictly normalizes native Trello card members as deterministic `id`, `username`, and `fullName` objects in `get`, `list`, and Inbox output. Without `--member`, list output remains normalized Work Units only. `list --member` searches every visible/open board card, returns ordinary cards and Work Units in the established discriminated shapes, matches an exact member ID or case-insensitive exact username, and rejects display-name and substring matching. Trello `visible` filtering excludes archived lists; explicit `closed` response filtering also excludes archived cards in open lists. Work Unit metadata filters exclude ordinary cards and compose conjunctively, while exact metadata `--owner` remains distinct. Native members remain plural Trello assignment/attention state; owner remains the single stable agent or worker execution claim, with no automatic synchronization or member mutation.
...
- `[VERIFIED]` The private `@jz/ai-arsenal-trello-work-cli` package is globally installed as `0.8.0`, exposing only `jz-trello-flow`. Its offline `template` command emits one canonical Draft with complete enum guidance, exactly one YAML fence, and unchanged local `validate --file` success without credentials, configuration, or Trello access. Artifact commit `957d8412da470686f6b42c90174ffe80b372014c` passed [Quality](https://github.com/jimzord12/ai-arsenal/actions/runs/30953856675) and [Windows/Linux Portability](https://github.com/jimzord12/ai-arsenal/actions/runs/30953856645). Its exact 32-file tarball SHA-256 is `68a38a394b892362c92ac78e36b70c2ca7504a41d6487d951daa68c5e87457e6`; every installed package file byte-matches the unpacked artifact, and repaired Git Bash, CMD, and PowerShell shims pass the required version/help/docs plus template-validation smoke. Exact `0.7.0` SHA-256 `bb8d22da2e8202c034823597aa4c5577dfe36b61cd2021074ca540a0daf10891` remains ready for rollback.
...
Issue #23 is delivered as private `jz-trello-flow` `0.8.0`; its exact CI-green artifact is globally installed and exact `0.7.0` remains the verified rollback. The next bounded request is issue #24, actionable `design start` input diagnostics and help.

...
- Mutating any Trello board other than explicitly authorized TestingBoard verification.
...
- Focused, package, repository, workflow, package-boundary, and authorized live verification pass without credential or repository artifacts leaking.
...
- Offline workbook validation plus one allowlisted clean-board live lifecycle that records and cleans only run-owned resources.
...
- Run strict publint/package validation, immediately remove generated tarballs, and prove none remain.
- After explicit live-test authority within implementation/verification, resolve exact TestingBoard ID `6a16bbf1fea5389eb39636b7`, verify the seven-list preconditions, exercise run-tagged production paths, and independently inspect cleanup postconditions.

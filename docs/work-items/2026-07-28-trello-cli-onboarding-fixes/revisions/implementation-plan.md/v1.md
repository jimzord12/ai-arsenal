...
- Test command: `pnpm --filter @jz/ai-arsenal-trello-work-cli exec jest --runInBand --runTestsByPath src/work-unit.test.ts src/config.test.ts src/transition.test.ts`
...
2. `pnpm --filter @jz/ai-arsenal-trello-work-cli exec jest --runInBand --runTestsByPath src/work-unit.test.ts src/config.test.ts src/transition.test.ts` — In Design schema, seven-list mapping, and Ready gating pass.

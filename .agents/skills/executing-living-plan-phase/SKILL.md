...
3. Return the router's eight-field brief and name its selected next skill. If
   the router reports structural corruption or malformed active metadata, stop
   at `initializing-living-plan-workflow` rather than beginning a phase.

## Boundaries

Do not create a work item, write an artifact, approve dangerous work, implement,
review, verify, deliver, release, pack, publish, globally install, delete source,
or invoke `features-cli`. A request to begin or resume work is not authority to
skip the router or validator-selected stage; the selected stages then continue
autonomously under repository policy.

For a valid no-active-item state, report the current `NEXT.md` action. Name
`define-monorepo-change` only after the user has described a new bounded
monorepo change.

...
- Trello Flow CLI `0.5.0` is the verified private release for native card-member output/filtering, distinct metadata-owner filtering, and self-contained bundled Agent Skill installation. Artifact commit `e9c005a2391b21c3add345203276bf720e1f48c6` passed exact-SHA Quality `30614651328` and Windows/Linux Portability `30614651510`; its exact 32-file SHA-256 `42e57a200b47d4bc020da186eacb7023d12148d1472cc12f422b9c94e4d88be0` artifact is globally installed and independently passes generated-shim help/docs, package-byte equality, member/owner fake-client behavior, and retained skill-installation gates without Trello access. Exact `0.4.2` rollback remains available.
- Trello Flow CLI source `0.5.1` is verified to return all matching visible/open ordinary cards and Work Units for `list --member`, exclude archived cards and archived-list cards, preserve normalized Work Unit-only output without `--member`, and keep metadata filters Work Unit-specific. Its active delivery item still requires exact-SHA CI, exact-commit packing, global replacement from installed `0.5.0`, byte proof, and read-only TestingBoard smoke before it becomes the installed release.

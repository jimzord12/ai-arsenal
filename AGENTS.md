...
Required CLI behavior work completes SemVer/Changesets release preparation in
implementation and records the generated package manifest and changelog before
review. Those package bytes and the release-preparation declaration remain
ordinary review-snapshot input. Delivery must not create/apply a Changeset or
edit package source, manifest, or changelog; any such repair resets all five
review fields and consumes another review cycle. An unsuccessful fourth cycle
blocks without an exemption or extra cycle.
...
1. During implementation, use SemVer to choose the package bump, create one Changeset, and apply it with `pnpm version-packages`; do not hand-edit package versions or changelogs. Preflight unrelated pending Changesets and block rather than consuming them. The configured Changesets workflow versions private packages and generates their changelogs.
2. Record the resulting package name, version, manifest, and sibling `CHANGELOG.md` as complete CLI release preparation before review. Include those generated bytes in the reviewed delivery snapshot, then run the full package gates, repository gates, packed-artifact validation, and a clean disposable-consumer installation from the exact generated tarball.
3. Delivery must not create/apply Changesets or edit package source, manifest, or changelog. After the exact reviewed snapshot is committed and pushed and its required GitHub Actions CI is successful, pack that exact CI-green commit and install or replace the resulting package in the Windows user's global pnpm environment. Do not globally link a mutable source tree.
...
8. After every global install, replacement, upgrade, downgrade, or rollback of `jz-trello-flow`, run `pnpm --dir <AI-Arsenal-repository-root> run repair:global-trello-shim`. Then, from outside the repository, require Git Bash `--version`, `--help`, and `docs --list` smoke checks plus CMD and PowerShell version checks before reporting the installation usable. Follow `docs/operations/trello-work-cli-global-install.md`; the generated extensionless pnpm shim can be regenerated with an MSYS-unsafe Bun target.
